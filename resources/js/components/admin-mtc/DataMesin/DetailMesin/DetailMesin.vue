    ...
    <div class="container-fluid mt-4">
      ...
      <div class="d-flex justify-content-between align-items-center mb-1">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
          <h1 class="h3" style="color: #000;"><b>Detail Mesin</b></h1>
          <div class="d-flex align-items-center justify-content-center">
            <div class="card shadow" style="max-width: auto; border-radius: 10px;">
              <div class="card-body text-center" style="border-radius: 10px; height: 30px;">
                <p>
                  <span class="m-2" style="color: #169ea8;"> Detail Mesin</span>/ 
                  <span class="mt-2 mb-2 mr-2 ml-1" style="color: #e6494b;">{{ mesin.kode_mesin || '-'}}</span>
                </p>
              </div>
            </div>
          </div>
            ...
            <div class="card shadow mb-0" style="border-radius: 20px;">
              <div class="card-body p-0" style="border-radius: 20px;"> <!-- Menghilangkan padding -->
                ...
                <table class="table no-border compact-table ml-2">              
                  <thead>
                    <tr>
                      <th class="text-left">Harga Pembelian Satuan Mesin</th>
                    </tr>                
                  </thead>
                  <tbody>
                    <tr>
                      <td class="text-">{{'Rp. ' + mesin.harga_beli || 'Rp. -' }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
        ...
        <div class="card-body ml-2" v-if="showDetail" style="border-radius: 20px;">
            <div class="col-md-12">
              <button 
                ...
                :class="{ active: showDetailMesin }" 
                @click="toggleDetailMesin"
              >
                <span v-if="showDetailMesin">Detail Mesin</span>
                <span v-else>Detail Mesin</span>
              </button>
              <button
                class="btn btn-show ml-1 mb-4 mr-1 mt-3"
                :class="{ active: showRincianMesin }"
                @click="toggleRincianMesin"
              >
                <span v-if="showRincianMesin">Detail Rincian Mesin</span>
                <span v-else>Detail Rincian Mesin</span>
              </button>
            </div>
          <div v-if="showDetailMesin" class="mt-3">
          <div class="col-md-4">
              <table class="table table-hover no-border compact-table">
                <tbody>
                  <tr>
                    <td>Kategori</td>
                    <th style="color: #000;">{{ mesin.kategori || '-' }}</th>
                  </tr>
                  <tr>                  
                    <td>Merek</td>
                    <th style="color: #000;">{{ mesin.merek_mesin || '-' }}</th>
                  </tr>
                  <tr>
                    <td>Type/Size</td>
                    <th style="color: #000;">{{ mesin.tipe_mesin || '-' }}</th>
                  </tr>
                  <tr>
                    <td>Available Stok</td>
                    <th style="color: #000">{{ mesin.stok_akhir || '-' }}</th>
                  </tr>
                  <tr>
                    <td>Produk</td>
                    <th style="color: #000;">{{ mesin.pembelian || '-' }}</th>
                  </tr>
                  <tr>
                    <td>Satuan</td>
                    <th style="color: #000">{{ mesin.unit_mesin || '-' }}</th>
                  </tr>
                  <tr>
                    <td>Sumber Mesin</td>
                    <th style="color: #000;">{{ mesin.sumber_mesin || '-' }}</th>
                  </tr>
                  <tr>
                    <td>Vendor</td>
                    <th style="color: #000;">{{ mesin.vendor || '-' }}</th>
                  </tr>                               
                </tbody>
              </table>
            </div>
            <div class="col-md-12">
              <table class="table table-hover no-border compact-table">
                <thead>                  
                  <tr>
                    <th style="color: #000;">Fungsi Mesin</th>                                      
                  </tr>
                </thead>
                <tbody>                  
                  <tr>
                    <td>{{ mesin.fungsi_mesin }}</td>                    
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div id="app" class="card-body" v-if="showRincianMesin" style="border-radius: 20px;">
            <data-rincian-mesin-belum-digunakan :kode-mesin="mesin.kode_mesin"></data-rincian-mesin-belum-digunakan>
          </div>
        </div>
        <!-- Konten Detail Alat Rusak -->
        <div id="app" class="card-body" v-if="showRincianAlatRusak" style="border-radius: 20px;">
          <data-detail-mesin-rusak :kode-mesin="mesin.kode_mesin"></data-detail-mesin-rusak>
        </div>
        <!-- Konten Detail Alat Musnah -->
        <div id="app" class="card-body" v-if="showRincianAlatMusnah" style="border-radius: 20px;">
          <data-detail-mesin-musnah :kode-mesin="mesin.kode_mesin"></data-detail-mesin-musnah>
        </div>
        <!-- Konten Detail Alat Error -->
        <div id="app" class="card-body" v-if="showRincianAlatError" style="border-radius: 20px;">
          <data-detail-mesin-error :kode-mesin="mesin.kode_mesin"></data-detail-mesin-error>
        </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        showRincianMesin: false, //Mengontrol tampilan rincian alat
    ...
    methods: {
      async fetchAlatDetail() {
        try {
          ...
          const response = await axios.get(`/api/mesins/${id}`);
          this.mesin = response.data; // Menyimpan data alat
      ...
      this.showRincianMesin = false;
      this.showRincianAlatError = false;
      this.showRincianAlatMusnah = false;
      this.showRincianAlatHilang = false;
      this.showDetailMesin = true;
      this.showDetail = !this.showDetail;
    },
    toggleDetailMesin(){
      this.showRincianMesin = false;
      this.showDetailMesin = !this.showDetailMesin;
    },
    toggleRincianMesin() {
      this.showDetailMesin = false;
      this.showRincianMesin = !this.showRincianMesin;
    },
