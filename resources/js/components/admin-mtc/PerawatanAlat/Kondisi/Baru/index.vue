<template>
  <div class="container-fluid">
    <h1 class="h3 mb-4 mt-4 text-gray-900"><b>Perawatan Alat/Mesin</b></h1>
    <ul id="pills-tab" role="tablist" class="nav nav-pills mb-3" style="margin-top: 1rem !important;">
      <li role="presentation" class="nav-item">
        <router-link id="pills-home-tab" data-toggle="pill" data-target="#pills-home" type="button" role="tab" aria-controls="pills-home" aria-selected="true" class="nav-link" :class="{ active: $route.name === 'baru-perawatan' }" :to="{ name: 'baru-perawatan' }">Baru</router-link>
      </li>
      <li role="presentation" class="nav-item">
        <router-link id="pills-profile-tab" data-toggle="pill" data-target="#pills-profile" type="button" role="tab" aria-controls="pills-profile" aria-selected="false" class="nav-link" :class="{ active: $route.name === 'proses-perawatan' }" :to="{ name: 'proses-perawatan' }">Proses</router-link>
      </li>
      <li role="presentation" class="nav-item">
        <router-link id="pills-contact-tab" data-toggle="pill" data-target="#pills-contact" type="button" role="tab" aria-controls="pills-contact" aria-selected="false" class="nav-link" :class="{ active: $route.name === 'selesai-perawatan' }" :to="{ name: 'selesai-perawatan' }">Selesai</router-link>
      </li>
    </ul>
      ...
      <table class="table table-border no-border table-custom" style="overflow-x: auto;">
        <thead>
          <tr class="bg-table text-center">
            ...
            <th class="text-black-1">No Perawatan</th>
            <th class="text-black-1">No Seri</th>
            <th class="text-black-1">Nama Alat/Mesin</th>
            <th class="text-black-1">Tgl Perawatan</th>
              ...
              <div class="dropdown text-center">
                ...
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                  <a class="dropdown-item" @click="perbaikanData(index)">
                    <i class="fas fa-check text-success"></i> Perawatan
                  </a>
                </div>
              </div>
            </td>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="form-group">
              <label for="tgl">Tgl Perawatan</label>
...
export default {
  ...
  data() {
    return {
      ...
      data: [
        { no_perawatan: 'R-01', no_seri: '1122wscj121', nama: 'Clamp', tgl: '2025-02-01', kondisi: 'Error', detail: 'Sensor tidak berfungsi', pic: 'John Doe', status: 'Belum' },
        { no_perawatan: 'R-02', no_seri: '1122wscj122', nama: 'Clamp', tgl: '2025-02-10', kondisi: 'Error', detail: 'Koneksi kabel terputus', pic: 'Jane Doe', status: 'Belum' },
      ],
      paginatedData: [],
      searchQuery: '',
      rowsPerPage: 5,
  ...
  methods: {
    ...
    perbaikanData(index) {
      Swal.fire({
        title: 'Konfirmasi',
        text: 'Apakah Anda yakin ingin memulai proses perawatan?',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'Ya, lakukan perawatan!',
        cancelButtonText: 'Tidak, batalkan!',
      }).then((result) => {
        if (result.isConfirmed) {
          this.data.splice(index, 1);
          this.updatePaginatedData();
          Swal.fire('Berhasil!', 'Data sedang dalam perawatan.', 'success');
        }
      });
    },
    prevPage() {
