          ...
          <table class="table table-border no-border table-custom" style="overflow-x: auto;">
            <thead>
              <tr class="bg-table text-center">
                ...
                <th class="text-center" style="width: 10px; color: #000;">No Kerusakan</th>
            ...
            <tbody v-for="(rynoseri, index) in filteredData" :key="index">
              <tr class="text-center">
                <td>{{ index + 1 }}</td>
                <td>{{ rynoseri.no_kerusakan }}</td>
                <td>{{ rynoseri.tgl_kerusakan || '-' }} <br> <small style="color: #444;"><i class="fas fa-clock"></i> {{ durasiData[index] !== '-' ? durasiData[index] + 'Hari' : '-' }}</small></td>
                <td>{{ rynoseri.kondisi || '-' }}</td>
                <td>{{ rynoseri.staff ? rynoseri.staff.nama_staff : '-' }}</td>
                <td>{{ rynoseri.detail_kerusakan || '-' }}</td>
              </tr>
            </tbody>
          </table>
          <!-- Pagination -->
  ...
  computed: {
    durasiData() {
      return this.datanoseri.map(noseri => {
        if (noseri.tgl_kerusakan) {
          const tanggal = new Date(noseri.tgl_kerusakan);
      ...
      if (
        ...
        return this.paginatedData.filter(datanoseri => {
          const tanggalMatch = this.tanggalAwal && this.tanggalAkhir
            ? datanoseri.tgl_kerusakan >= this.tanggalAwal && datanoseri.tgl_kerusakan <= this.tanggalAkhir
            : true;

          const searchMatch = this.searchQuery
            ? (datanoseri.detail_kerusakan && datanoseri.detail_kerusakan.toLowerCase().includes(this.searchQuery.toLowerCase())) ||
            (datanoseri.no_kerusakan && datanoseri.no_kerusakan.toLowerCase().includes(this.searchQuery.toLowerCase()))
            : true;
  ...
  methods: {
    async fetchNoSeriAlat() {
      try {
        ...
        const response = await axios.get(`/api/v1/kerusakan/getRusak/${noSeri}`);
