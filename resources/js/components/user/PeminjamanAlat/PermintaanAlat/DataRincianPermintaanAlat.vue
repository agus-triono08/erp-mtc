                  ...
                  <td class="text-center">
                    <div class="dropdown text-center">
                      ...
                      <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">                
                        <a class="dropdown-item" @click="setStatus(item, 'Digunakan')">
  ...
  methods: {
    ...
    updateStatusToDiambil() {
      ...
      const ditolakItems = this.selectedNoSeri.filter(item =>
        ...
        (item.kondisi_after === 'Ditolak' || item.kondisi_after === 'Belum Diproses')
      );
      ...
      Swal.fire({
        title: 'Yakin?',
        text: 'Semua item terpilih akan diubah menjadi "Diambil".',
        icon: 'question',
        ...
        if (result.isConfirmed) {
          axios.post('/api/v1/noseri/bulk-update-status-permintaan-user', {
    ...
    setStatus(item, status, reason = '') {
      // Cek jika status saat ini sudah "Ditolak"
      if (['Ditolak', 'Belum Diproses'].includes(item.kondisi_after)) {
      ...
      axios.post('/api/v1/noseri/update-status-permintaan-user', {
        id: item.id,
        status: status,
        reason: reason
      })
    ...
    openRejectModal(item) {
      if (item.kondisi_after === 'Menunggu Diambil') {
        Swal.fire({
          ...
          text: `No Seri ${item.no_seri} sedang dalam status "Menunggu Diambil" dan tidak dapat ditolak.`,
        });
