        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="form-group">
              ...
              <input type="date" class="form-control" v-model="form.tgl_kembali" required />
            </div>
            <div class="form-group">
              <label for="keterangan_perubahan" style="color: #000;">
                <b>Alasan Perubahan Alat/Mesin</b>
                <sup style="color: red;">*</sup>
              </label>
              <div class="textarea-wrapper">
                <textarea
                  id="keterangan_perubahan"
                  v-model="form.keterangan_perubahan"
                  class="form-control"
                  rows="3"
                  placeholder="Masukkan Alasan Perubahan Alat/Mesin Nya (Maksimal 100 Karakter)"
                  maxlength="100"
                ></textarea>
                <small class="text-muted char-counter">
                  {{ form.keterangan_perubahan.length }} / 100
                </small>
              </div>
            </div>
          </div>
  ...
  methods: {
    ...
    async submitForm() {
      try {
        ...
        axios.post(`/api/v1/perubahan-peminjaman/${this.noPinjam}`, this.form)
        
        // Close the modal and reset form on success
        this.tutupModal();
        this.$emit('form-submitted', response.data); // Emit event if needed for parent
      } catch (error) {
    ...
    openModal(action, item = null, index = null) {
      if (action === 'add') {
        ...
        this.form.keterangan_perubahan = '';
    ...
    saveData() {
      if (this.currentIndex === null) {
        axios.post(`/api/v1/perubahan-peminjaman/store/${this.noPinjam}`, this.form)
        .then(response => {
