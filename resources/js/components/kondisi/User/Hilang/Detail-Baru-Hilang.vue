...
export default {
  ...
  methods: {  
    ...
    async addAktivitas() {
      ...
      // Validasi wajib
      if (!this.aktivitas.buktiPertanggungJawaban) {
        Swal.fire({
          icon: 'warning',
      ...
      formData.append('bukti_pertanggung_jawaban', this.aktivitas.buktiPertanggungJawaban);

      try {
        const res = await fetch('/api/v1/kehilangan/add-activity', {
          method: 'POST',
    ...
    downloadPJ() {
      ...
      const filename = `PJ_${this.dataBaru.id}.pdf`;
      ...
      const alat = this.dataBaru.no_seri?.tools?.nama || '-';
      const noseri = this.dataBaru.no_seri?.no_seri || '-';
