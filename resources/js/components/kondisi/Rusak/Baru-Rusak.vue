  ...
  <div class="container-fluid">
    ...
    <ul class="nav nav-tabs" id="myTab" role="tablist">
        <li class="nav-item" role="presentation">
          <router-link class="nav-link" id="kategori-tab" data-toggle="tab" role="tab" aria-controls="kategori" aria-selected="true" :class="{ active: $route.name === 'kondisi-baru-rusak' }" :to="{ name: 'kondisi-baru-rusak' }">Baru</router-link>
        </li>
        <li class="nav-item" role="presentation">
        ...
        <li class="nav-item" role="presentation">
          <router-link class="nav-link" id="kategori-tab" data-toggle="tab" role="tab" aria-controls="kategori" aria-selected="false" :class="{ active: $route.name === 'kondisi-selesai-musnah' }" :to="{ name: 'kondisi-selesai-musnah' }">Selesai</router-link>
        </li>
      </ul>
            ...
            <td>
              <div class="dropdown text-center">
                ...
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                  <router-link :to="{ name: 'kondisi-detail-baru-rusak', params: { id: item.id } }" class="dropdown-item">
...
export default {
  ...
  computed: {
    filteredData() {
      let result = this.dataKerusakan.filter(item => {
        return (
    ...
    durasidata() {
      return this.dataKerusakan.map(item => {
  ...
  methods: {
    async fetchData() {
      const res = await fetch('/api/v1/kerusakan');
      const data = await res.json();
      this.dataKerusakan = data.byStatusBaru;
      // console.log(this.dataKerusakan);
    },
    ...
    debouncedFetchNoSeri: _.debounce(function () {
      ...
      let filteredData = this.dataKerusakan;
      
      if (this.startDate && this.endDate) {
        const start = new Date(this.startDate);
        const end = new Date(this.endDate);
        
        filteredData = this.dataKerusakan.filter(item => {
          if (!item.tgl_kerusakan) return false;
          const itemDate = new Date(item.tgl_kerusakan);
      ...
      const rows = filteredData.map((item, index) => {
        const tglKerusakan = item.tgl_kerusakan 
          ? new Date(item.tgl_kerusakan).toLocaleDateString('id-ID') 
          : '-';
