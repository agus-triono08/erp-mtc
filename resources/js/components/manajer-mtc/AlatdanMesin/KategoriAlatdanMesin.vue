  ...
  <div class="container-fluid">
    ...
    <ul class="nav nav-tabs" id="myTab" role="tablist">
      <li class="nav-item" role="presentation">
        <router-link class="nav-link" id="kategori-tab" data-toggle="tab" role="tab" aria-controls="kategori" aria-selected="true" :class="{active: $route.name === 'kategori-mgn'}" :to="{name: 'kategori-mgn'}">Kategori Alat/Mesin</router-link>
      </li>
      <li class="nav-item" role="presentation">
        <router-link class="nav-link" id="merek-tab" data-toggle="tab" role="tab" aria-controls="merek" aria-selected="false" :class="{active: $route.name === 'merek-mgn'}" :to="{name: 'merek-mgn'}">Merek Alat/Mesin</router-link>
      </li>
      <li class="nav-item" role="presentation">
        <router-link class="nav-link" id="tipe-tab" data-toggle="tab" role="tab" aria-controls="tipe" aria-selected="false" :class="{active: $route.name === 'tipe-mgn'}" :to="{name: 'tipe-mgn'}">Tipe Alat/Mesin</router-link>
      </li>
    </ul>
      ...
      <table class="table table-border no-border table-custom" style="overflow-x: auto;">
        <thead>
          <tr class="bg-table text-center">
            ...
            <th @click="sortBy('jenis')" style="cursor: pointer;">
              Jenis
              <i class="fas" :class="{
                'fa-sort-up': sortKey === 'jenis' && sortDirection === 'asc',
                'fa-sort-down': sortKey === 'jenis' && sortDirection === 'desc'
              }"></i>
            </th>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control" id="nama" v-model="form.nama_kategori">
            </div>            
          </div>
...
export default {
  ...
  computed: {
    filteredData() {
      let result = this.Kategori.filter(item => {
        return (
          item.jenis?.nama_jenis?.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
          item.kode_kategori.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
  ...
  methods: {
    ...
    openModal(action, item = null, index = null) {
      if (action === 'add') {
        ...
        this.form.jenis = item.jenis_id;
        this.form.kode_kategori = item.kode_kategori;
    ...
    saveData() {
      const payload = {
        jenis_id: this.form.jenis, // karena form.jenis sekarang menyimpan ID
      ...
      if (this.currentIndex === null) {
        ...
        axios.post('/api/v1/kategori', payload)
          .then(response => {
            this.Kategori.push(response.data);
            this.closeModal();
            this.fetchData();
        ...
        axios.put(`/api/v1/kategori/${this.currentId}`, payload)
          .then(response => {
            this.Kategori[this.currentIndex] = response.data;
            this.closeModal();
            this.fetchData();
            this.currentPage = 1; // Tambahkan kode ini untuk reset pagination
          })
